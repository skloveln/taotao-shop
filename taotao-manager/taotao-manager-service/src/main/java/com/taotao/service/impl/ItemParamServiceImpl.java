...
public class ItemParamServiceImpl implements ItemParamService {
	...
	@Override
	public EasyUIDataGridResult getItemParemList(Integer page, Integer rows) {
		
		//分页处理
		PageHelper.startPage(page, rows);
		
		//执行查询
		List list = itemParamMapper.selectTbItemParam();
	...
	@Override
	public TaotaoResult deleteItemParem(long id) {
		if(itemParamMapper.deleteByPrimaryKey(id)>0){
			return new TaotaoResult().ok();
		}else{
			return new TaotaoResult().build(500, "deleteFail");
		}	
	}
